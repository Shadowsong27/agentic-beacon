...
`abc warehouse init` pre-populates `skills/record-knowledge/` — a skill for capturing decisions, lessons, and facts into the knowledge base. Projects that sync it (via `abc sync` or `abc install skills/record-knowledge`) can use `/record-knowledge` to record knowledge directly from the agent.
