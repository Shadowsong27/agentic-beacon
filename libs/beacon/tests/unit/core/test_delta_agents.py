"""Tests for Phase 7: DeltaStatus.STALE, agent_live_path, compare_agent_file.
...
TDD Test Cases for agent_live_path (7.2):
...
TDD Test Cases for compare_agent_file (7.4):
...
def testagent_live_path_opencode(tmp_path, fake_home):
    """TC1: agent="opencode" → ~/.config/opencode/agents/code-reviewer.md"""
    comp, _, _ = _make_comparator(tmp_path)
    result = comp.agent_live_path("opencode", "agents/code-reviewer.md")
    expected = fake_home / ".config" / "opencode" / "agents" / "code-reviewer.md"
    assert result == expected


def testagent_live_path_claudecode(tmp_path, fake_home):
    """TC2: agent="claudecode" → ~/.claude/agents/code-reviewer.md"""
    comp, _, _ = _make_comparator(tmp_path)
    result = comp.agent_live_path("claudecode", "agents/code-reviewer.md")
    expected = fake_home / ".claude" / "agents" / "code-reviewer.md"
    assert result == expected


def testagent_live_path_strips_agents_prefix_only(tmp_path, fake_home):
    """TC3: Nested path "agents/sub/name.md" → strips only agents/ prefix."""
    comp, _, _ = _make_comparator(tmp_path)
    result = comp.agent_live_path("opencode", "agents/sub/name.md")
    expected = fake_home / ".config" / "opencode" / "agents" / "sub" / "name.md"
    assert result == expected


# ---------------------------------------------------------------------------
# 7.4 compare_agent_file
# ---------------------------------------------------------------------------
...
def testcompare_agent_file_missing(tmp_path, fake_home):
    """TC1: Global file absent → MISSING."""
    ...
    result = comp.compare_agent_file("agents/code-reviewer.md")

    assert result.agent_statuses["opencode"] == DeltaStatus.MISSING


def testcompare_agent_file_identical(tmp_path, fake_home):
    """TC2: Global file identical to warehouse → IDENTICAL."""
    comp, warehouse, _ = _make_comparator(tmp_path)
    (warehouse / "agents" / "code-reviewer.md").write_text(AGENT_CONTENT)
...
def testcompare_agent_file_modified(tmp_path, fake_home):
    """TC3: Global file differs from warehouse → MODIFIED."""
    comp, warehouse, _ = _make_comparator(tmp_path)
    (warehouse / "agents" / "code-reviewer.md").write_text(AGENT_CONTENT)
...
def testcompare_agent_file_no_tools(tmp_path, fake_home):
    """TC4: No tools detected (agents_paths empty) → empty result, no error."""
    comp, warehouse, _ = _make_comparator(tmp_path)
    (warehouse / "agents" / "code-reviewer.md").write_text(AGENT_CONTENT)
    comp.agents_paths = {}
