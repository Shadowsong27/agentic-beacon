# copy-to-symlink-migration Specification

## Purpose
Define the one-shot migration path that `abc sync` follows when it encounters a pre-existing copy-based `.agentic-beacon/artifacts/` tree (artifacts stored as regular files from the old snapshot model). Migration surfaces any local edits against the warehouse, lets the user choose to contribute or discard them, and then replaces each regular file with a symlink into the warehouse clone. The behavior is opt-in through interactive prompts or explicit non-interactive flags; it never silently discards local work.
