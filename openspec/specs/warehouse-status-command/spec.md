# warehouse-status-command Specification

## Purpose
Define the `abc warehouse status` command, the read-side counterpart to `abc warehouse contribute`. From any beacon-configured project directory, it reports uncommitted and unpushed state inside the warehouse clone, scoped to artifacts referenced by the current project's `beacon.yaml`. It replaces the removed `abc delta` command; under the symlink model there is no project-vs-warehouse drift to diff, but users still need visibility into warehouse-side pending changes that they (or other projects on the same machine) have accumulated.
