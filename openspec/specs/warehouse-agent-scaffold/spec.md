...
The initializer SHALL write a `README.md` into the `agents/` directory explaining the markdown frontmatter format and the `abc install agents/<name>.md` workflow. The README is written only when no `README.md` already exists.
...
The `examples/sample-warehouse/` included in the repository SHALL contain an `agents/README.md` matching the current template, so that the sample warehouse stays consistent with the `abc warehouse init` output.
