# symlink-based-sync Specification

## Purpose
Define the symlink-based distribution model for warehouse artifacts. `abc sync` materializes each artifact declared in `beacon.yaml` as a filesystem symlink from the project's `.agentic-beacon/artifacts/` tree into the locally cloned warehouse, so there is exactly one physical file per logical artifact per machine. This replaces the previous copy-based snapshot model and eliminates the silent last-writer-wins regressions that arose when two projects held independent copies of the same file.
