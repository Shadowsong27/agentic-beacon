...
**Output**: Every skill in `skills/*/SKILL.md` carries a validated `requires:` block. All `TODO: verify` flags resolved. Changes committed with a conventional commit message.
**Validation**: `grep -L 'requires:' skills/*/SKILL.md` returns empty (every skill has the key). For every name listed in any `requires.contexts`, a matching `contexts/<name>.md` exists. `git log -1 --format=%s` on the warehouse shows a conventional-commit message describing the frontmatter migration.
...
**Goal**: Update `ArtifactsConfig` to remove `knowledge`; wire the legacy-drop migration so old `beacon.yaml` files sync cleanly once.
...
**Output**: `ArtifactsConfig` has no `knowledge`; loader strips any legacy `artifacts.knowledge` key from parsed YAML with a one-shot info log; writer never serializes the field.
...
<!-- opsx:tdd:3.4:begin -->
  ...
  - **TDD Test Cases (write these first):**
    ...
    - TC2: If model is permissive, writer filters → round-trip produces no `knowledge:` line
     - TC3: Manifest constructed with all defaults → written YAML contains `contexts`, `skills` keys only
<!-- opsx:tdd:3.4:end -->
...
<!-- opsx:tdd:4.2:end -->
- [x] 4.3 Define `SkillFrontmatter` Pydantic model with `requires` validation rules from spec
<!-- opsx:tdd:4.3:begin -->
  - **Input**: Parsed frontmatter dicts for skills (with `requires.contexts`).
  - **Expected Output**: `SkillFrontmatter.model_validate(dict)` succeeds for valid shapes and fails with clear errors for invalid ones.
  - **Validation**: Pydantic ValidationError raised on missing keys, wrong types, forbidden keys.
  - **TDD Test Cases (write these first):**
    - TC1: Skill with `requires: { contexts: [foo] }` → validates
    - TC2: Skill with `requires: { contexts: [] }` → validates
    - TC3: Skill missing `requires` entirely → ValidationError identifying the missing key
    - TC4: Skill with `requires: { contexts: [], skills: [] }` → ValidationError: `skills` not permitted on skills
    - TC5: Skill with `requires.contexts: 'foo'` (string not list) → ValidationError
    - TC6: Skill with duplicate entries `requires.contexts: [foo, foo]` → deduplicated silently
<!-- opsx:tdd:4.3:end -->
...
<!-- opsx:tdd:4.4:end -->
- [x] 4.5 Implement `validate_requires_against_warehouse(frontmatter, warehouse_path)` — each context name must resolve to an existing warehouse file
<!-- opsx:tdd:4.5:begin -->
  - **Input**: Parsed skill frontmatter object + `Path` to a warehouse clone.
  - **Expected Output**: `list[ValidationError]` — empty on success, one entry per missing target on failure. Each error includes the referring skill, the missing context name, and the expected warehouse path.
  - **Validation**: Fixture warehouse with known contents; assert validator returns expected error list.
  - **TDD Test Cases (write these first):**
    - TC1: Skill `requires.contexts: [python-standards]`, warehouse has `contexts/python-standards.md` → empty error list
    - TC2: Skill `requires.contexts: [missing]`, warehouse lacks `contexts/missing.md` → one error naming 'missing' and the expected path
    - TC3: Skill `requires.contexts: [python-standards, testing]`, warehouse has first but not second → single error for `testing`
    - TC4: Empty requires.contexts → empty error list (not an error)
<!-- opsx:tdd:4.5:end -->
- [x] 4.6 Unit tests: valid skill, missing `requires`, malformed YAML, skill with forbidden `skills` key, dangling reference
<!-- opsx:tdd:4.6:begin -->
...
- [x] 6.2 Implement `compute_effective_set(beacon, warehouse) -> EffectiveSet` where `EffectiveSet` contains explicit contexts, adopted skills, transitive contexts (from skill requires), and derived knowledge
<!-- opsx:tdd:6.2:begin -->
  - **Input**: A `BeaconManifest` (with contexts and skills) and a connected warehouse fixture.
  ...
  - **TDD Test Cases (write these first):**
    ...
    - TC2: One explicit context, no skill → contexts={that}, skills=∅, knowledge=(whatever that context refs)
    - TC3: One skill requiring one context → contexts contain the context even though not explicitly adopted
...
- [x] 6.5 Collect missing-dependency errors into a list; return structured failure when non-empty
<!-- opsx:tdd:6.5:begin -->
  - **Input**: Manifest with one adopted skill requiring three contexts, two of which are missing.
  ...
  - **TDD Test Cases (write these first):**
    ...
    - TC2: Two missing contexts in same skill → failure with 2 errors, collected in one pass
    - TC3: Missing context required by a transitively-pulled skill → failure with 1 error, path chain in message
<!-- opsx:tdd:6.5:end -->
- [x] 6.6 Provide a pure function `is_transitively_required(artifact, effective_set) -> bool` for use in pruning decisions
<!-- opsx:tdd:6.6:begin -->
...
**Validation**: Integration test against a fixture warehouse exercising skills (explicit), contexts (explicit + transitive from skill requires), and knowledge (derived); post-sync tree matches expected symlink set exactly; running sync twice produces an idempotent tree; provoked missing-dep error contains the migration-doc path.
...
**Output**: Every scaffolded skill carries `requires:` frontmatter; no `knowledge:` list appears in the scaffolded `beacon.yaml` template.
